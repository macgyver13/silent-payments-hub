 ...
 */
function computeECDHSharedSecrets(inputPublicKeys: string[], scanPrivateKey: Buffer): Buffer[] {
    const sharedSecrets: Buffer[] = [];
    
    for (const inputPubkeyHex of inputPublicKeys) {
        // Parse the input public key from the transaction
        const inputPubkey = Buffer.from(inputPubkeyHex, 'hex');
        
        // Compute ECDH: shared_secret = scan_private_key * input_public_key
        const sharedSecretPoint = ecc.pointMultiply(inputPubkey, scanPrivateKey) as Uint8Array;
        const sharedSecretBytes = Buffer.from(sharedSecretPoint);
        
        console.log("shared_secret:", utils.uint8ArrayToHex(sharedSecretBytes));
...
const ecdhSharedSecrets = computeECDHSharedSecrets(inputPublicKeysForBlock, scanPrivateKey);
            ...
            if (scriptPubkey.length === 34 && scriptPubkey.subarray(0, 2).equals(Buffer.from("5120", "hex"))) {
                ...
                if (actualOutputPubkey.equals(expectedOutputPubkey)) {
                    ...
                    matchedUtxos.push({
                        ...
                        vout: output.vout,
                        value: output.value,
                        outputKeyTweak: candidateKey.outputKeyTweak,
                        outputPubKey: candidateKey.outputPubkey,
...
if (matchedUtxos.length > 0) {
    for (const utxo of matchedUtxos) {
        console.log(`Matched UTXO: txid=${utxo.txid}, vout=${utxo.vout}, value=${utxo.value}, tweak=${utxo.outputKeyTweak}, output_key=${utils.uint8ArrayToHex(utxo.outputPubKey)}`);
    }
} else {
