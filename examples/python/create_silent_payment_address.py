...
def create_labeled_silent_payment_address(
    b_scan: ECKey, scan_pubkey: ECPubKey, spend_pubkey: ECPubKey, m: int, hrp: str = "tsp"
) -> str:
...
def seed_to_silent_payment_address(
    ...
    Args:
        ...
        account: BIP32 hardened account index (default: 0)
    ...
    if account < 0:
        raise ValueError(f"Invalid account: {account}. Must be a non-negative integer")
    ...
    base_path = f"352'/{coin_type}'/{account}'"

    # BIP352: Silent Payments derivation path m/352'/coin_type'/account'
    purpose_node = master_key.ChildKey(352 + 2**31)  # 352'
    ...
    change_address = create_labeled_silent_payment_address(
        ECKey().set(scan_privkey_bytes), scan_pubkey, spend_pubkey, 0, hrp
    )
    print(f"Change address (label 0): {change_address}")
    ...
    desc_external_base = f"sp([{master_fingerprint}/{base_path}]{spscan_encoded})"
    desc_internal_base = f"sp([{master_fingerprint}/{base_path}]{spspend_encoded})"
...
def print_detailed_info(
    seed_phrase: str, network: str = "testnet", labels: list[int] = None, account: int = 0
) -> None:
    """Print detailed information about the Silent Payment derivation process."""

    print("=== Silent Payment Address Generation ===")
    print(f"Seed phrase: {seed_phrase}")
    print(f"Network: {network}")
    print(f"Account: {account}")
    print(f"Derivation path: m/352'/{'0' if network == 'mainnet' else '1'}'/{account}'")
    print(f"Scan path: m/352'/{'0' if network == 'mainnet' else '1'}'/{account}'/1'/0")
    print(f"Spend path: m/352'/{'0' if network == 'mainnet' else '1'}'/{account}'/0'/0")
    if labels:
        print(f"Labels: {sorted(labels)}")
    print()

    seed_to_silent_payment_address(seed_phrase, network=network, labels=labels, account=account)
...
Examples:
  ...
  python create_silent_payment_address.py --seed "your seed phrase here" --network testnet
  python create_silent_payment_address.py --seed "your seed phrase here" --network testnet --labels 1-10,15,20-25
  python create_silent_payment_address.py --seed "your seed phrase here" --network mainnet --account 2
        """,
    )
    ...
    parser.add_argument(
        "--network", default="testnet", choices=["mainnet", "testnet"], help="Target network (default: testnet)"
    )
    parser.add_argument("--account", type=int, default=0, help="BIP32 hardened account index (default: 0)")
    parser.add_argument(
        "--labels", type=str, help="Label ranges and/or individual labels (e.g., '1-10,15,20-25' or '1,3,5')"
    )
    ...
    print_detailed_info(args.seed.strip(), args.network, labels, args.account)
